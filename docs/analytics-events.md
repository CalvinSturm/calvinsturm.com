...
- `cta_location`: `hero`, `hero_secondary`, `top`, `inline`, `final`, or `product_cta`. The homepage (`source_path: /`) also sends `header` (the header download button, FastCast) `store` (the product panel under the 3D shelf, any product) and `shelf` (the button on the centre tile of the 3D shelf) for `<product>_download_clicked`, `fastcast_license_clicked` and `fastplay_github_clicked`.
